...
// 0.38 sits it a bit lower (closer to the trigger) within that free area.
// Module-scope (like FALL_Y) since it never varies for the page lifetime.
const CENTER_Y = -((CONSTANTS.containerSize / 2 - CONSTANTS.itemSize / 2) * 0.38);
...
export function CircleMenu({ items, activeId, onSelect, centerItem }) {
  ...
  // The center button isn't part of the arc, so include it when resolving the
  // active item — otherwise the closed trigger shows the wrong icon/color while
  // the admin tab is active.
  const activeItem =
    (centerItem && centerItem.id === activeId && centerItem) ||
    visibleItems.find((it) => it.id === activeId) ||
    visibleItems[0];
